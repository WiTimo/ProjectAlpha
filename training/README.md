...
- `--resolutions`: Optional list of resolutions to combine (e.g. `--resolutions fast mid slow`). The first entry becomes the
  base timeline and slower windows are aligned via causal `merge_asof` joins before forward filling within each day.
...
- `--enable-regime-training`: Train and persist one TCN per volatility regime (detected via `volatility_regime` by default).
- `--regime-column`: Override which column defines regimes when multi-regime training is enabled.
- `--regime-values`: Explicit list of regime identifiers to train when you do not want to rely on dataset inference.
- `--min-regime-rows`: Guardrail that skips regimes with fewer rows than the provided threshold.
- `--model-output-dir`: Destination for regime bundles (`state_dict`, scaler stats, manifest) generated during multi-regime runs.
